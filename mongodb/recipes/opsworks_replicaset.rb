...
if !node.recipe?("mongodb::shard")
  ...
  replicaset_name = node['opsworks']['instance']['layers'].first
  replicaset_layer_instances = node['opsworks']['layers'][replicaset_name]['instances']

  Chef::ResourceDefinitionList::MongoDB.configure_replicaset(node, replicaset_name, replicaset_members(node, replicaset_layer_instances))
end
